...
def main():
    ...
    # draw_watermark_lines(image, angle=45, color=(0, 0, 255, 128), shadow_color=(0, 0, 0, 128), spacing=spacing, emboss=True)  # 蓝色半透明
    ...
    # draw_watermark_lines(image, angle=135, color=(255, 0, 0, 128), shadow_color=(0, 0, 0, 128), spacing=spacing, emboss=True)  # 红色半透明
    intersections = []
    # 计算所有交点
    ...
    # # 保存图片
    image.save("watermark_image_with_emboss.png")
    print("水印图片已保存为 watermark_image_with_emboss.png")
    rotated_np = np.array(image)  # 转换为 (H,W,4) 的uint8数组
