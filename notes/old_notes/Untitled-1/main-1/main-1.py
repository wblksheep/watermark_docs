...
def draw_dashed_line(draw, start, end, color, shadow_color, dash_length=10, gap_length=5, width=2, offset=2, negtive=False):
    ...
    while current_distance < distance:
        ...
        draw.line(
            ...
            fill=shadow_color, width=width+3
        )
        # draw.line(
        #     [(x1 + dx_unit * current_distance+offset, y1 + dy_unit * current_distance+negtive_1*offset),
        #      (x1 + dx_unit * next_distance+offset, y1 + dy_unit * next_distance+negtive_1*offset)],
...
def draw_watermark_lines(image, angle, color, shadow_color, line_width=6, spacing=50, emboss=True):
    """
    在图片上绘制指定角度的水印线
    :param image: PIL图片对象
    ...
    if angle == 45:
        ...
        #     # 绘制主虚线
            draw_dashed_line(draw, (i, 0), (i + height, height), color,shadow_color, width=line_width, negtive=False)
            if index%2==0:
        ...
        for i in range(0, width + height, spacing):
            ...
            # 绘制主虚线
            draw_dashed_line(draw, (i, 0), (i - height, height), color,shadow_color, width=line_width, negtive=True)
            if index%2==0:
...
def main():
    ...
    background_color = (255, 255, 255, 0)  # 白色半透明背景
    ...
    # 绘制45度水印线
    draw_watermark_lines(image, angle=45, color=(255, 255, 255, opacity), shadow_color=(0, 0, 0, shadow_opacity), spacing=spacing, emboss=True, line_width=line_width)  # 白色半透明
    # draw_watermark_lines(image, angle=45, color=(255, 0, 0, opacity), shadow_color=(0, 0, 0, shadow_opacity), spacing=spacing, emboss=True, line_width=line_width)  # 白色半透明
    draw_foggy(image_foggy,angle=45,color=(255, 255, 255, 255),line_width=foggy_line_width, spacing=spacing)

    # 绘制135度水印线
    # draw_watermark_lines(image, angle=135, color=(255, 255, 255, 128), spacing=50, emboss=True)  # 红色半透明
    draw_watermark_lines(image, angle=135, color=(255, 255, 255, opacity), shadow_color=(0, 0, 0, shadow_opacity), spacing=spacing, emboss=True, line_width=line_width)  # 白色半透明
    # draw_watermark_lines(image, angle=135, color=(0, 0, 255, opacity), shadow_color=(0, 0, 0, shadow_opacity), spacing=spacing, emboss=True, line_width=line_width)  # 白色半透明
    draw_foggy(image_foggy,angle=135,color=(255, 255, 255, 255),line_width=foggy_line_width, spacing=spacing)
    intersections = []
    # 计算所有交点
    for i in range(len(angle_135)):
        for j in range(len(angle_45)):
            ...
            if intersection:
                ...
                x =x-text_width/2
                y=y-text_height/2
                draw.text((x + 2, y + 2), watermark_text, font=font, fill=(0, 0, 0, shadow_opacity))
                draw.text((x - 2, y - 2), watermark_text, font=font, fill=(0, 0, 0, shadow_opacity))
                draw.text((x - 2, y + 2), watermark_text, font=font, fill=(0, 0, 0, shadow_opacity))
                draw.text((x + 2, y - 2), watermark_text, font=font, fill=(0, 0, 0, shadow_opacity))
    ...
    # 保存图片
    final_image.save("watermark_image_with_emboss.png")


    print("水印图片已保存为 watermark_image_with_emboss.png")
